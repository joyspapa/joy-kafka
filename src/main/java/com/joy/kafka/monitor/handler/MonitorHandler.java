...
public class MonitorHandler {
	...
	protected String getTopicNamebyGroupID(String groupID) {
		ListConsumerGroupOffsetsResult listConsumerGroupOffsetsResult = KafkaAdminClientFactory
				.getAdminClient(getBrokers()).listConsumerGroupOffsets(groupID);
		try {
			Set<TopicPartition> topicPartitionSet = listConsumerGroupOffsetsResult.partitionsToOffsetAndMetadata().get()
					.keySet();

			for (TopicPartition t : topicPartitionSet) {
				return t.topic();
			}
			return "none";
		} catch (Exception ex) {
			logger.warn("getTopicNamebyGroupID warning. groupID : " + groupID, ex);
			return "none";
		}
	}
